...
export function LoginPage(){
    ...
    const getTabClasses = (tabName: 'signin' | 'signup') => {
    ...
    const getTextColor = (tabName: 'signin' | 'signup') => {
        return activeGender === tabName ? 'text-white' : 'text-[#A8A2A2]';
    };
    const getGender = (tabName: 'Male' | 'Female') => {
        const baseClasses = "h-[90%] w-[50%] rounded-full flex items-center justify-center transition-colors duration-300";
    ...
    const getGenderTextColor = (tabName: 'Male' | 'Female') => {
    ...
    const Signin = () => {
        return(
            ...
            <div className="p-1 flex flex-col w-full h-full gap-2 items-center">
                ...
                <div className='flex mt-5 h-[40px] w-[90%] border border-[#405673] rounded-full bg-transparent items-center justify-between'>
                        <button 
                ...
                <div className="flex flex-col w-[90%]  gap-5 mt-5">
                    <div className="flex justify-between items-center h-[50px] w-full border-transparent border border-[#415876] bg-transparent rounded-full">
                        <input
                            placeholder="Enter your email"
                            type="text"
                            className="w-full h-full outline-none placeholder-white pl-5 bg-transparent"
                        />
                        <Mail className="h-10 w-14 pr-5 text-white" />
                    </div>
                    <div className="flex justify-between items-center h-[50px] w-full border-transparent border border-[#415978] bg-transparent rounded-full">
                        <input
                            placeholder="Enter your Password"
                            type="text"
                            className="w-full h-full  placeholder-white pl-5 bg-transparent"
                        />
    ...
    const Signup = () => {
        return(
            <div className="p-1 flex flex-col w-full h-full gap-1 items-center">
                <div className="w-full ml-4">
                ...
                <div className="flex flex-col w-[90%] items-center gap-2">
                    <div className='flex gap-4 h-[40px] w-full '>
                        <input
                            ...
                            className=" w-full lg:w-[60%] mx-auto pl-3 border  border-[#405673] rounded-full  bg-transparent"
                        />
                        <input
                    ...
                    <div className='flex h-[40px] w-full border border-[#405673] rounded-full bg-transparent items-center justify-between'>
                        <button 
                            ...
                            >
                            <h1 className={`text-sm font-bold ${getGenderTextColor('Male')}`}>Sign In</h1>
                        </button>
                        <button 
                            onClick={() => {setActiveGender('Female'); setRetate(false);}}
                            className={getGender('Female')}
                        >
                            <h1 className={`text-sm font-bold ${getGenderTextColor('Female')}`}>Sign Up</h1>
                        </button>
                    </div>
                    <input
                            placeholder="Enter Your Email"
                            className="h-[40px] w-full mx-auto pl-3 border border-[#405673] rounded-full  bg-transparent"
                        />
                    <input
                            placeholder="Enter Your Password"
                            className="h-[40px] w-full mx-auto pl-3 border  border-[#405673] rounded-full  bg-transparent"
    ...
    return(
        <div className={`w-full h-[700px] lg:h-[457px] lg:w-[441px] p-4 lg:p-0 border border-[#5F88B8] rounded-lg bg-[#09122C] mx-auto flex items-center justify-center overflow-hidden`}>
