...
const CreateMcq = ({ onSuccess }: CreateMcqProps) => {
    ...
    return (
        <form 
            ...
            className='flex flex-col gap-4 divide-y-2 divide-gray-200 dark:divide-gray-800 p-5 border border-gray-300 dark:border-gray-800 rounded-lg overflow-hidden bg-surface-main dark:bg-secondary-darkbg transition-colors duration-300'
        >
            <div className='flex justify-between'>
                <div className='py-2 px-2 lg:px-4 flex gap-2 bg-slate-300/20 dark:bg-slate-700/30 w-fit rounded-lg'>
                    <CopyCheck className='text-black dark:text-surface-main' />
                    <h1 className='text-sm lg:text-lg font-semibold text-black dark:text-surface-main'>Multiple Choice</h1>
                </div>
                
                <select {...register("difficulty")} className="py-2 px-2 lg:px-4 flex gap-2 bg-slate-300/20 
                    dark:bg-slate-700/30 text-black dark:text-surface-main w-fit rounded-lg outline-none border-none">
            ...
            {[
                ...
                <div key={field.name} className='flex flex-col gap-2 pt-5'>
                    <label className='flex gap-1 items-center'>
                        <div className='h-6 w-6 bg-black dark:bg-primary text-center text-surface-main 
                            rounded-md flex items-center justify-center text-xs'>?</div>
                        <h1 className='text-black dark:text-surface-main font-medium capitalize'>
                            {field.label} 
                    ...
                    {field.type === "textarea" ? (
                        <textarea
                            ...
                            className="w-full bg-slate-100/50 dark:bg-slate-800/50 text-black dark:text-surface-main 
                                border border-gray-200 dark:border-gray-700 rounded-lg p-4 outline-none min-h-[100px] focus:border-primary"
                        />
                    ) : (
                        <input 
                            type='text'
            ...
            <div className='flex flex-col gap-2 pt-5'>
                <h1 className='text-black dark:text-surface-main font-medium'>Choices <span className='text-red-500'>* (Need 4)</span></h1>
            ...
            {/* Tags */}
            <div className='flex flex-col gap-2 pt-5'>
                <h1 className='text-black dark:text-surface-main font-semibold'>Tags</h1>
    ...
    return (
        <div className='flex gap-2 items-center w-full'>
            ...
            <input 
                ...
                className='flex-1 h-12 bg-slate-100/50 dark:bg-slate-800/50 text-black dark:text-surface-main p-3 
                    border border-gray-200 dark:border-gray-700 rounded-lg outline-none focus:border-primary'
            />
            <button 
                type='button' 
                onClick={() => { if(text){ onAdd(text, checkBox); setText(""); setCheckBox(false); }}} 
                className='h-12 px-4 flex gap-2 items-center bg-surface-main dark:bg-secondary-darkbg border-2 border-dashed 
                    border-gray-300 dark:border-gray-700 rounded-lg text-black dark:text-surface-main 
    ...
    <div className='flex flex-col gap-2'>
        ...
        <div className='flex items-center gap-2 py-2 px-4 rounded-lg bg-slate-100/50 
            ...
            <input
                ...
                className='bg-transparent outline-none w-16 text-black dark:text-surface-main font-bold'
            />
