      ...
      <div className='h-auto w-full max-w-[500px] bg-[#121b31]/50 p-6 rounded-2xl border border-[#1e2e52] max-h-[85vh] overflow-y-auto custom-scrollbar shadow-2xl'>
        <form onSubmit={handleSubmit(ApplySubmit)} className='flex flex-col gap-4 w-full'>
            <div className='flex-1'>
              <input type='text' {...register("fullName")} placeholder="fullName" className={inputClass} />
              {errors.fullName && <p className="mt-1 text-red-500 text-[10px]">{errors.fullName.message}</p>}
            </div>

          <div className='flex flex-col sm:flex-row gap-3'>
            <div className='flex-1'>
              <input type='text'{...register("phoneNumber", { valueAsNumber: true })} placeholder="phoneNumber" className={inputClass} />
          ...
          <div className='flex flex-col sm:flex-row gap-3'>
            <div className='flex-1'>
              <input type='url' {...register("linkedinUrl")} placeholder="linkedinUrl" className={inputClass} />
              {errors.linkedinUrl && <p className="mt-1 text-red-500 text-[10px]">{errors.linkedinUrl.message}</p>}
            </div>
            <div className='flex-1'>
              <input type='url' {...register("portfolioUrl")} placeholder="portfolioUrl" className={inputClass} />
          ...
          <label 
            ...
            <input id="cv-upload" type="file" accept="application/pdf" {...register("cv")} hidden />
          </label>
