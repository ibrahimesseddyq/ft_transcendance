          ...
          {loading ? (
            ...
            <>
              ...
              <ul className="flex flex-col gap-1.5">
                {phases.sort((a, b) => a.orderIndex - b.orderIndex).map((phase) => (
                  <li key={phase.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 group">
                    ...
                    <button onClick={() => handleDelete(phase.id)} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 transition-all">
                      <Trash2 size={12} />
                    </button>
                  </li>
                ))}
              ...
              {showForm ? (
                <form onSubmit={handleSubmit}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => setShowForm(false)} className="h-8 px-3 text-xs font-semibold text-gray-500">
                      Cancel
                    </button>
                  </div>
                </form>
