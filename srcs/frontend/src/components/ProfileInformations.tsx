  ...
  return (
    <form onSubmit={handleSubmit(onApplySubmit)} className="max-w-screen-2xl p-6 overflow-y-auto custom-scrollbar bg-transparent w-full mx-auto">
      ...
      <div className="relative group mx-auto my-8 w-32 h-32">
        ...
        <label htmlFor="avatar-input" className="absolute bottom-1 right-1 p-2 bg-white dark:bg-gray-800 rounded-full shadow-lg cursor-pointer border border-gray-200 dark:border-gray-700 hover:scale-110 transition-transform">
      ...
      <footer className="flex mt-8 gap-2 justify-end">
        <button 
          ...
          className="w-full md:w-48 bg-[#00adef] hover:bg-[#0086b8] disabled:bg-gray-400 text-white font-bold py-3 px-6 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2"
        >
          {isSubmitting ? <><Loader2 className="animate-spin h-4 w-4" /> Saving...</> : "Save Profile"}
        </button>
        {profile ? (
          <button type="button" onClick={() => navigate(-1)} className="font-semibold py-3 px-6 text-black dark:text-white hover:text-red-500">
            Cancel
          </button>
        ) : (
          <Logout />
