...
dev: clear
	...
	cd srcs/backend/main_service && npm install && npm run dev && npx prisma generate && npx prisma db push &
	# cd srcs/backend/quiz_service && npm install && npm run dev && npx prisma generate && npx prisma db push  &
...
clear: clean-dev
	...
	-@sudo kill -9 $$(lsof -t -i:3306) 2>/dev/null || true
	-@sudo kill -9 $$(lsof -t -i:3307) 2>/dev/null || true
	@echo "Done."
